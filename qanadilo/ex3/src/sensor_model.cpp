...
//    cv::findNonZero(lane_regression, idnex)
//    std::cout << "The MAGIC ANSWER:  " << lane_regression.at(0).at<int>(0,0)<<std::endl;
//    std::vector<cv::Mat> nonZeroCoordinates;
//
//    std::cout << "before successful qanadilo" << std::endl;
//    cv::findNonZero(lane_regression, nonZeroCoordinates);
//    std::cout << "successful qanadilo" << std::endl;
//    for (int i = 0; i < nonZeroCoordinates.total(); i++ ) {
//        cout << "Zero#" << i << ": " << nonZeroCoordinates.at<Point>(i).x << ", " << nonZeroCoordinates.at<Point>(i).y << endl;
    float temp_weight = 0;
    const float no_need = 0;
    std::vector<Eigen::Vector3f> corr_center_points = getNearestPoints(center_points);

    for (int i_p = 0; i_p < particles.size(); i_p++) {
        success = true;

        // visualizer_->SendPoses(particles, "particles", "map");
        ...
        for (int j_o = 0; j_o < world_signs.size(); j_o++) {
            ...
            //std::cout << particles[i_p].transform.matrix() << std::endl;
            //std::cout << " weight " << particles[i_p].weight << std::endl;
            //std::cout << " temp_weight " << temp_weight<< std::endl;

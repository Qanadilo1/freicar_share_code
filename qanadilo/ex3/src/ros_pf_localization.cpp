...
//    std::cout << "The MAGIC ANSWER:  " << latest_lane_reg_.second.toSec() <<std::endl;
//    std::cout << "The MAGIC ANSWER2:  " << last_odo_update_.toSec() <<std::endl;
...
//    cv::threshold(reg, reg, 30, 255,3);
    ...
    for (int i=0; i<600; i++)
    {
        for (int j=0; j<600; j++)
        {
            std::cout << "The MAGIC PIXELS:  " <<reg.at<int>(i,j)<<std::endl;
        }

...
void particle_filter::LowVarianceSampling(){
    ...
    std::random_device rd;
    std::mt19937 gen(rd());
    r = (entry(gen)*total_weight)/NUM_PARTICLES;
    //std::discrete_distribution<int> entry(0, 1);
    std::vector<Particle> resampled;
    //rrr = (rand() / (float)RAND_MAX) * (1.0 / (float)NUM_PARTICLES);
...
void particle_filter::InitParticles() {
    ...
    for (int i = 0; i < NUM_PARTICLES; i++) {
        ...
        p.weight = 1.0;
        Eigen::Transform<float,3,Eigen::Affine> t = Eigen::Transform<float,3,Eigen::Affine>::Identity();
        // std::cout << t.matrix()(0,0) << std::endl;
    ...
    particles_init_ = true;
        ...
        //Eigen::Transform<float,3,Eigen::Affine> t = Eigen::Transform<float,3,Eigen::Affine>::Identity();
        //float x = float(rand()%(maxes_mins[1]-maxes_mins[0] + 1);
        //Eigen::Vector3f add_t(4, 4, 0.0f);
        //t.translate(add_t);
        //dummy.transform = t;
...
void particle_filter::ConstantVelMotionModel(nav_msgs::Odometry odometry, float time_step) {
    ...
    if ((odometry.twist.twist.angular.z) > 0.1) {
        motion_factor_1 = odometry.twist.twist.linear.x + odometry.twist.twist.linear.y  / odometry.twist.twist.angular.z;
    } else {
        motion_factor_1 = odometry.twist.twist.linear.x + odometry.twist.twist.linear.y  ;
    }
    double motion_factor_2 = odometry.twist.twist.angular.z * time_step;
    */
     if(particles_init_) {
        Particle temp_particles;
        // init particles_ vector with NUM_Particles
        for (int i = 0; i < NUM_PARTICLES; i++) {
            // temp_particles.transform = particles_[i].transform;
            //Eigen::Transform<float,3,Eigen::Affine> s = particles_[i].transform;
            Eigen::Quaternionf rot;
            // &p.transform.matrix()(0,0) = p.transform.matrix()(0,0) + velocity + gausion
            rot = Eigen::AngleAxisf(0.0, Eigen::Vector3f::UnitX())
                  * Eigen::AngleAxisf(0.0, Eigen::Vector3f::UnitY())
                  * Eigen::AngleAxisf((odometry.twist.twist.angular.z - dist_yaw(generator_)) * time_step, Eigen::Vector3f::UnitZ());
            // = particle.x + velocity + noise
            ...
            Eigen::Vector3f add_t((odometry.twist.twist.linear.x - dist_x(generator_)) * time_step , (odometry.twist.twist.linear.y - dist_y(generator_))* time_step,
                                  0.0f);
           // temp_particles.transform.translate(add_t);
            //temp_particles.transform.rotate(rot);
            particles_[i].transform.translate(add_t); // = temp_particles.transform;
            particles_[i].transform.rotate(rot);
            //particles_.push_back(particles_[i]);

            //particles_.pop_back(); //show last time_step
            //particles_[i].transform = temp_particles.transform;
